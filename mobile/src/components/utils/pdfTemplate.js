export const generateAnalyticsReportHtml = (BOOKS_DATA, totalAccess, chartUri) => {
  ...
  const fastestBook = BOOKS_DATA.reduce((min, book) =>
    book.book_read_time && book.book_read_time < min.book_read_time ? book : min,
    BOOKS_DATA[0]
  );
  ...
  const interpretationText = (() => {
    ...
    if (fastestBook.book_title === mostAccessedBook.book_title) {
      text += `Interestingly, this same book is also the fastest to read (${fastestBook.book_read_time} minutes), showing strong engagement.`;
    } else if (fastestBook.book_title === leastAccessedBook.book_title) {
      text += `Surprisingly, the least accessed book is also the fastest to read (${fastestBook.book_read_time} minutes), suggesting it may be overlooked despite being quick to complete.`;
    } else {
      text += `The fastest book to read is <strong>${fastestBook.book_title}</strong> (${fastestBook.book_read_time} minutes), suggesting shorter books may encourage quicker completion.`;
    }
    return text;
  })();
  ...
  return `
    <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
      ...
      <ul>
        ${BOOKS_DATA.map(book => 
          `<li>${book.book_title} — Access Count: ${book.book_count} (Avg Read: ${book.book_read_time || 'N/A'} mins)</li>`
        ).join('')}
      </ul>
      ...
      <ul>
        ...
        <li><strong>Fastest Book Read:</strong> ${fastestBook.book_title} (${fastestBook.book_read_time} mins)</li>
      </ul>
